...
---
## 2026-06-22 — Weekly progress check
...
## Schedule
- **Plan target:** Day 30 of 30 (submission target was ~2026-05-29 — 24 calendar days ago)
- **Actual progress:** Day 26 of 30 (last code shipped 2026-05-13 — 40 days ago)
- **Variance:** 4 plan-days behind; no code or external actions completed in 40 days
...
## What's next (the critical path)
1. **Legal pages (privacy policy + ToS) — non-code, ~2 hours, overdue by 5+ weeks.** Both stores reject submissions without live URLs at `cookable.app/privacy` and `cookable.app/terms`. Use any free generator (termly.io, freeprivacypolicy.com) and host on the existing domain. This single action unblocks Play Console submission.
2. **Apple Developer account ($99/yr) purchase or commit to Android-only launch.** Hard blocker for iOS TestFlight (Day 28) and App Store submission (Day 30). The 24-day slip past the original target makes Android-first a reasonable decision to make now rather than continuing to defer.
3. **App Store + Play Store assets (Days 26-27)** — first remaining code task; listing copy from `knowledge/11-app-store-listing.md`, 10 screenshots in all required sizes, iOS Privacy Nutrition Labels, Android Data Safety section, swap AdMob test IDs to prod in `eas.json`.

## Pre-launch checklist
- Accounts: 7 / 9 complete
- Legal: 0 / 5 complete
- Brand assets: 5 / 7 complete
- App readiness: 10 / 13 complete
- Pre-launch testing: 0 / 4 complete
- Distribution prep: 0 / 8 complete

## Risks / blockers
- **40 days since last code shipped (2026-05-13)** — fourth consecutive weekly snapshot with zero progress; momentum and motivation risk now rivals schedule risk
- **Submission target passed 24 days ago (2026-05-29)** — 4 plan-days of work (Days 27-30) remain entirely unstarted; no external gates from prior snapshots have been cleared
- **Apple Developer account ($99/yr) still not purchased** — blocks iOS TestFlight and App Store submission with no workaround; deferred since Day 1 on financial grounds
- **Legal pages (privacy policy + ToS) not hosted** — both stores will reject without live URLs; flagged in every snapshot for 5+ weeks with no action; this is a 2-hour task
- **Play Console internal-testing track + IAP products not created** — `cookable_plus_monthly` / `cookable_plus_annual` products not set up; blocks Android paywall sandbox testing; no code required
- **AdMob prod interstitial unit IDs missing from `eas.json`** — must be added before the Day 28 production build
- **Distribution prep at 0 / 8** — landing page, email waitlist, social handles, and TikTok content entirely unstarted; 40 days of pre-launch audience-building time has been lost

## Suggested focus this week
Four consecutive weekly snapshots have reported zero movement against identical blockers. The code through Day 26 is complete and sound; the four remaining plan-days (store assets, beta, submission) are mechanical and can be executed in a focused 3-5 day sprint the moment the external gates open. Those gates are: (1) legal pages — a two-hour task with no dependencies, overdue by over a month; (2) the iOS decision — purchase the Apple Developer account this week or commit in writing to an Android-first launch so the plan can adjust. Making either decision today and acting on the legal pages immediately would unblock the entire remaining build. Distribution prep is also at zero and represents an independent launch-day risk; even claiming one social handle or setting up a basic waitlist page on the existing domain would be meaningful progress. The window for a June launch is still open but closes this week.

---

## 2026-06-29 — Weekly progress check
...
## Schedule
- **Plan target:** Day 30 of 30 (submission target was ~2026-05-29 — 31 calendar days ago)
- **Actual progress:** Day 26 of 30 (last code shipped 2026-05-13 — 47 days ago)
- **Variance:** 4 days behind; no code or external actions completed in 47 days
...
## What's next (the critical path)
1. **Legal pages (privacy policy + ToS) — non-code, ~2 hours, overdue by 6+ weeks.** Both stores reject submissions without live URLs at `cookable.app/privacy` and `cookable.app/terms`. Use any free generator (termly.io, freeprivacypolicy.com) and host on the existing domain. This single action unblocks Play Console submission.
2. **Apple Developer account ($99/yr) purchase or commit to Android-only launch.** Hard blocker for iOS TestFlight (Day 28) and App Store submission (Day 30). Five consecutive snapshots have flagged this; the decision must be made this week rather than deferred again.
3. **App Store + Play Store assets (Days 26-27)** — first remaining code task; listing copy from `knowledge/11-app-store-listing.md`, 10 screenshots in all required sizes, iOS Privacy Nutrition Labels, Android Data Safety section, swap AdMob test IDs to prod in `eas.json`.
...
## Risks / blockers
- **47 days since last code shipped (2026-05-13)** — fifth consecutive weekly snapshot with zero progress; the build has been stalled since mid-May
- **Submission target passed 31 days ago (2026-05-29)** — 4 plan-days of work (Days 27-30) remain entirely unstarted; no external gates from any prior snapshot have been cleared
- **Apple Developer account ($99/yr) still not purchased** — blocks iOS TestFlight and App Store submission with no workaround; if not resolved this week, Android-only is the only viable launch path
- **Legal pages (privacy policy + ToS) not hosted** — both stores will reject submissions without live URLs; flagged in every snapshot for 6+ weeks with no action; this is a 2-hour task with no code or cost dependency
- **Play Console internal-testing track + IAP products not created** — `cookable_plus_monthly` / `cookable_plus_annual` not set up; blocks Android paywall sandbox testing; no code required
- **AdMob prod interstitial unit IDs missing from `eas.json`** — must be added before the Day 28 production build
- **Distribution prep at 0 / 8** — landing page, email waitlist, social handles, and TikTok content entirely unstarted; 47 days of pre-launch audience-building opportunity has passed

## Suggested focus this week
Five consecutive weekly snapshots, identical blockers, zero movement. The code through Day 26 is complete; four plan-days of work (store assets, beta, submission) separate it from launch. Those four days cannot start without two non-code actions: host the legal pages today (two hours, any free generator, no cost) and make the iOS decision — purchase the Apple Developer account this week or commit to Android-first in writing. With both gates open, a focused 3-5 day sprint can reach Play Console internal testing and, if iOS is live, TestFlight before mid-July. The distribution side is at zero and represents a separate launch-day risk that compounds every idle week; even one action (a waitlist page, one social handle claimed, one TikTok draft) reduces that exposure. A July launch is still achievable. It requires a decision today.

---

## 2026-07-06 — Weekly progress check
...
## Schedule
- **Plan target:** Day 30 of 30 (submission target was ~2026-05-29 — 38 calendar days ago)
- **Actual progress:** Day 26 of 30 (last code shipped 2026-05-13 — 54 days ago)
- **Variance:** 4 days behind on plan-days; submission window has passed by 5+ weeks with no external gates cleared

## What's done
- **Week 1 (Days 1-7):** Expo SDK 54 scaffold, brand theme, 4-tab nav, Apple + Google auth (Google validated; Apple pending account), camera + resize + Supabase Storage upload, migrations 001-002
...
## What's next (the critical path)
1. **Legal pages (privacy policy + ToS) — non-code, ~2 hours, overdue by 6+ weeks.** Both stores reject submissions without live URLs at `cookable.app/privacy` and `cookable.app/terms`. Use any free generator (termly.io, freeprivacypolicy.com) and host on the existing domain. Unblocks Play Console submission immediately.
2. **Apple Developer account ($99/yr) purchase or a written commitment to Android-only launch.** Hard blocker for iOS TestFlight (Day 28) and App Store submission (Day 30). Six snapshots have flagged this; deferral is no longer a strategy.
3. **App Store + Play Store assets (Day 27)** — the next code task; listing copy from `knowledge/11-app-store-listing.md`, 10 screenshots in all required sizes, iOS Privacy Nutrition Labels, Android Data Safety section, swap AdMob test IDs to prod in `eas.json`.
...
## Risks / blockers
- **54 days since last code shipped (2026-05-13)** — sixth consecutive weekly snapshot with zero progress; the build has been stalled since mid-May
- **Submission target passed 38 days ago (2026-05-29)** — 4 plan-days of work (store assets, beta, submission) remain entirely unstarted; no external gates have been cleared in any prior snapshot
- **Apple Developer account ($99/yr) still not purchased** — blocks iOS TestFlight and App Store submission; if not resolved this week, Android-only is the only viable path to any launch
- **Legal pages (privacy policy + ToS) not hosted** — both stores will reject without live URLs; flagged in every snapshot since late May; a 2-hour task with no cost dependency
- **Play Console internal-testing track + IAP products not created** — `cookable_plus_monthly` / `cookable_plus_annual` not set up; blocks Android paywall sandbox testing; no code required
- **AdMob prod interstitial unit IDs missing from `eas.json`** — must be added before the Day 28 production build
- **Distribution prep at 0 / 8** — landing page, email waitlist, social handles, and TikTok content entirely unstarted; 54 days of pre-launch audience-building opportunity has passed

## Suggested focus this week
Six consecutive snapshots, zero movement on any external gate. The situation has not changed in six weeks: the code through Day 26 is complete and sound; the only things standing between the current state and a Play Console submission are two non-code actions (legal pages — two hours, free) and two dashboard tasks (Play Console IAP products and internal-testing track — one hour, no cost). A July launch on Android is still achievable if both are done this week and the Day 27 code work (screenshots, store assets) follows immediately. iOS launch is contingent on the Apple Developer account decision — that call has to be made now rather than deferred to another snapshot. If financial constraints make the $99 genuinely impossible, commit to Android-first in writing and update CLAUDE.md accordingly so the plan can stop branching on an open question. On distribution: 54 idle days is real damage. A single focused hour this week — claim one social handle, put up a one-page waitlist on the domain — starts to rebuild the launch runway. The window for any meaningful pre-launch audience is closing; acting on the two non-code gates today is the only way to create time for distribution work before the app ships.

---

## 2026-07-13 — Weekly progress check
...
## Schedule
- **Plan target:** Day 30 of 30 (submission target was ~2026-05-29 — 45 calendar days ago)
- **Actual progress:** Day 26 of 30 (last code shipped 2026-05-13 — 61 days ago)
- **Variance:** 4 plan-days behind; submission window has passed by 6+ weeks; seventh consecutive snapshot with zero progress
...
## What's next (the critical path)
1. **Legal pages (privacy policy + ToS) — non-code, ~2 hours, overdue by 7+ weeks.** Both stores reject submissions without live URLs at `cookable.app/privacy` and `cookable.app/terms`. Use any free generator (termly.io, freeprivacypolicy.com) and host on the existing domain. This single action unblocks Play Console submission immediately.
2. **Apple Developer account ($99/yr) purchase or a written commitment to Android-only launch.** Hard blocker for iOS TestFlight (Day 28) and App Store submission (Day 30). Seven snapshots have flagged this; continued deferral means a July launch is Android-only at best.
...
## Risks / blockers
- **61 days since last code shipped (2026-05-13)** — seventh consecutive weekly snapshot with zero progress; the build has been stalled for over two months
- **Submission target passed 45 days ago (2026-05-29)** — 4 plan-days of work (store assets, beta, submission) remain entirely unstarted; no external gates have been cleared in any prior snapshot
...
- **Legal pages (privacy policy + ToS) not hosted** — both stores will reject without live URLs at `cookable.app/privacy` and `cookable.app/terms`; flagged in every snapshot since late May; this is a 2-hour task with no cost dependency that has been overdue for seven weeks
...
- **Distribution prep at 0 / 8** — landing page, email waitlist, social handles, and TikTok content entirely unstarted; 61 days of pre-launch audience-building opportunity has been lost
...
---
## 2026-07-27 — Weekly progress check
...
## Schedule
- **Plan target:** Day 30 of 30 (submission target was ~2026-05-29 — 59 calendar days ago)
- **Actual progress:** Day 26 of 30 (last code shipped 2026-05-13 — 75 days ago)
- **Variance:** 4 plan-days behind; ninth consecutive weekly snapshot with zero progress
...
## What's next (the critical path)
1. **Legal pages (privacy policy + ToS) — non-code, ~2 hours, overdue by 9 weeks.** Both stores reject submissions without live URLs at `cookable.app/privacy` and `cookable.app/terms`. Use any free generator (termly.io, freeprivacypolicy.com) and host on the existing domain. This single action unblocks Play Console submission immediately.
2. **Apple Developer account ($99/yr) purchase or a written commitment to Android-only launch.** Hard blocker for iOS TestFlight (Day 28) and App Store submission (Day 30). Nine snapshots have flagged this; a July Android launch is the only remaining viable window and it closes this week.
3. **App Store + Play Store assets (Day 27)** — the next code task once legal pages are live; listing copy from `knowledge/11-app-store-listing.md`, 10 screenshots in all required sizes, iOS Privacy Nutrition Labels, Android Data Safety section, swap AdMob test IDs to prod in `eas.json`.
...
## Risks / blockers
- **75 days since last code shipped (2026-05-13)** — ninth consecutive weekly snapshot with zero progress; the build has been stalled for 10+ weeks
- **Submission target passed 59 days ago (2026-05-29)** — 4 plan-days of work (store assets, beta, submission) remain entirely unstarted; no external gates have been cleared in any prior snapshot
- **Apple Developer account ($99/yr) still not purchased** — blocks iOS TestFlight and App Store submission with no workaround; if not resolved immediately, Android-only is the only viable launch path; this decision must be committed to in writing
- **Legal pages (privacy policy + ToS) not hosted** — both stores will reject without live URLs at `cookable.app/privacy` and `cookable.app/terms`; flagged in every snapshot since late May; a 2-hour task with no cost dependency overdue for nine weeks
- **Play Console internal-testing track + IAP products not created** — `cookable_plus_monthly` / `cookable_plus_annual` not set up in Play Console; blocks Android paywall sandbox testing before beta; no code required
- **AdMob prod interstitial unit IDs missing from `eas.json`** — banner IDs wired; interstitial IDs must be added before the Day 28 production build
- **Distribution prep at 0 / 8** — landing page, email waitlist, social handles, and TikTok content entirely unstarted; 75 days of pre-launch audience-building opportunity has been lost

## Suggested focus this week
Nine consecutive snapshots, zero movement. The code through Day 26 is complete; four mechanical plan-days (store assets, beta builds, submission) are all that remain before the app can ship. Both gates blocking those four days are non-code tasks: host the legal pages today (two hours, any free generator, no cost — this is the ninth time this has been the top action item) and commit to iOS vs Android-only in writing so the plan can stop branching on an open question. A Play Console submission is achievable in August if both gates open this week and the Day 27 sprint follows immediately. A meaningful July window has passed. On distribution: 75 idle days without a waitlist, landing page, or social presence is now a serious launch-day risk that no amount of code quality can offset — even one action (a static waitlist page on the existing domain, one social handle claimed) starts to rebuild that runway. The app is ready. The blockers are two hours of admin and one decision.
